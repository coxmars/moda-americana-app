...
$(document).ready(function() {
	...
	tablaProductos = $('#dataTableUsers').DataTable({
		...
		buttons: [
			{
				extend: "excelHtml5", filename: `TablaUsuarios-${jsDate}`, text: "Excel", titleAttr: "Exportar Excel", className: "btn btn-success px-3 mr-2 rounded",
				messageTop: `Registro de Usuarios`,
				exportOptions: {
					columns: [1, 2, 3, 4, 5, 6]
			...
			{
				extend: "pdfHtml5", filename: `TablaUsuarios-${jsDate}`, text: "PDF", titleAttr: "Exportar PDF", className: "btn btn-danger px-3 mr-2 rounded",
				exportOptions: {
					columns: [1, 2, 3, 4, 5, 6]
				...
				customize: function(doc) {
					...
					doc['header'] = (function() {
						return {
							columns: [
								{ alignment: 'center', italics: false, text: 'Registro de Usuarios', color: "black", fontSize: 18, margin: [0, 10] },
							],
							margin: 20
						}
					});
			...
			{
				extend: "print", filename: "Usuarios", text: "Imprimir", titleAttr: "Imprimir", className: "btn btn-dark px-2 mr-2 rounded",
				exportOptions: {
					columns: [1, 2, 4, 5, 7, 8]
				}
			}
